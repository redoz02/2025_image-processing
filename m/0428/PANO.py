...
class Panorama(QMainWindow):
    ...
    def collectFunction(self):
        # 영상 수집 버튼 누르면 나머지 기능 비활성화
        self.showButton.setEnabled(False)
        self.stitchButton.setEnabled(False)
        self.saveButton.setEnabled(False)
        self.label.setText('c를 여러 번 눌러 수집하고 끝나면 q를 눌러 비디오를 끝냅니다.')

        self.cap = cv.VideoCapture(0, cv.CAP_DSHOW)
        if not self.cap.isOpened():
            sys.exit('카메라 연결 실패')

        self.imgs = []  # 수집한 프레임 리스트
        ...
        while True:
            ...
            if key == ord('c'):
                self.imgs.append(frame)  # 프레임 수집
            elif key == ord('q'):
                self.cap.release()
                cv.destroyWindow('video display')
                break

        # 두 장 이상 수집했을 경우 다음 기능 활성화
        if len(self.imgs) >= 2:
